...
async def put_artigo(artigo_id: int, artigo: ArtigoSchema, usuario_logado: UsuarioModel = Depends(UsuarioModel), db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(ArtigoModel).filter(ArtigoModel.id == artigo_id)
        result = await session.execute(query)
...
async def delete_artigo(artigo_id: int, usuario_logado: UsuarioModel = Depends(UsuarioModel), db: AsyncSession = Depends(get_session)):
    async with db as session:
